<script lang="ts">
  ...
  import type { CameraDevice, CaptureSession, PublisherStats } from "../lib/media";
  ...
  export let publisherStats: PublisherStats | null = null;
  ...
  afterUpdate(() => {
    const output = capture?.stream ?? null;
    if (previewVideo && previewVideo.srcObject !== output) {
      previewVideo.srcObject = output;
      if (output) void previewVideo.play().catch(() => undefined);
    }
...
<div class="live-page min-h-dvh pb-24 md:pb-0">
  <div class="live-shell mx-auto w-full max-w-6xl px-3 py-4 sm:px-6 sm:py-6 lg:px-8">
    ...
    <div class="live-content-grid grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(290px,0.65fr)]">
      ...
      <aside class="live-sidebar space-y-4"><section class="panel p-4" aria-labelledby="stats-heading"><div class="mb-4 flex items-center gap-2 text-[var(--accent)]"><Wifi size={18} /><span class="mono text-xs font-bold uppercase tracking-[0.14em]">TELEMETRY</span></div><h2 id="stats-heading" class="text-xl font-extrabold">Stream stats</h2><dl class="mt-4 grid grid-cols-2 gap-x-4 gap-y-3"><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">Max video</dt><dd class="mono mt-1 text-lg font-extrabold">{formatBitrate(stream.maxBitrateKbps)}</dd></div><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">Transport</dt><dd class="mono mt-1 text-lg font-extrabold">WHIP</dd></div><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">Browser sent</dt><dd class="mono mt-1 text-lg font-extrabold">{formatBitrate(publisherStats?.videoBitrateKbps)}</dd></div><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">Diterima</dt><dd class="mono mt-1 text-lg font-extrabold">{formatBitrate(stats?.receivedBitrateKbps)}</dd></div><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">RTP lost</dt><dd class="mono mt-1 text-lg font-extrabold">{publisherStats?.videoPacketsLost ?? "-"}</dd></div><div><dt class="text-xs font-bold uppercase tracking-[0.1em] text-[var(--faint)]">Readers</dt><dd class="mono mt-1 text-lg font-extrabold">{stats?.srtReaders ?? 0} / 10</dd></div></dl><div class="mt-4 border-t border-[var(--border)] pt-3 text-sm text-[var(--muted)]"><p class="flex items-center justify-between gap-3"><span>Video encoder</span><strong class="mono text-[var(--text)]">{stream.codec.toUpperCase()}</strong></p><p class="mt-2 flex items-center justify-between gap-3"><span>Browser encoder</span><strong class="mono max-w-[62%] truncate text-[var(--text)]" title={publisherStats?.encoderImplementation || "-"}>{publisherStats?.encoderImplementation || "-"}</strong></p><p class="mt-2 flex items-center justify-between gap-3"><span>ICE route</span><strong class="mono text-[var(--text)]">{publisherStats ? `${publisherStats.protocol} / ${publisherStats.localCandidateType}` : "-"}</strong></p><p class="mt-2 flex items-center justify-between gap-3"><span>Quality limit</span><strong class="mono text-[var(--text)]">{publisherStats?.qualityLimitationReason || "-"}</strong></p><p class="mt-2 flex items-center justify-between gap-3"><span>Audio encoder</span><strong class="mono text-[var(--text)]">{stream.audioEnabled ? stream.audioCodec.toUpperCase() : "OFF"}</strong></p><p class="mt-2 flex items-center justify-between gap-3"><span>Recording</span><strong class="inline-flex items-center gap-1.5 text-[var(--text)]"><HardDrive size={15} />{(stats?.recording ?? stream.record) ? "server on" : "off"}</strong></p></div></section><section class="panel p-4" aria-labelledby="srt-heading"><div class="mb-4 flex items-center gap-2 text-[var(--accent)]"><Radio size={18} /><span class="mono text-xs font-bold uppercase tracking-[0.14em]">OBS INPUT</span></div><h2 id="srt-heading" class="text-xl font-extrabold">SRT read URL</h2><p class="mt-1.5 text-sm leading-6 text-[var(--muted)]">URL tetap sama saat relay di-Stop dan Start ulang.</p><textarea class="field mono mt-3 min-h-[100px] w-full resize-y p-3 text-xs leading-5" readonly aria-label="SRT read URL">{stream.srtUrl || "URL belum tersedia"}</textarea><button class="button-secondary mt-2 flex w-full items-center justify-center gap-2" type="button" on:click={onCopy} disabled={!stream.srtUrl}>{#if copied}<Check size={17} class="text-[var(--success)]" /><span>Tersalin</span>{:else}<Copy size={17} /><span>Copy SRT URL</span>{/if}</button></section></aside>
    </div>
  </div>
