<script lang="ts">
  import { Activity, KeyRound, LogOut, Plus, RefreshCw, Server, ShieldCheck, Wifi } from "@lucide/svelte";
  ...
  $: activeStreams = streams.filter((stream) => stream.status === "connecting" || stream.status === "live");
</script>
...
<div class="min-h-dvh">
  ...
  <main class="mx-auto w-full max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <section class="mb-7 grid gap-4 sm:grid-cols-3" aria-label="Ringkasan server">
      <div class="panel p-5"><div class="mb-4 flex items-center justify-between"><span class="text-xs font-bold uppercase tracking-[0.12em] text-[var(--faint)]">Active streams</span><Activity size={18} class="text-[var(--accent)]" /></div><p class="mono text-3xl font-extrabold">{activeStreams.length}<span class="text-base text-[var(--faint)]"> / 8</span></p><p class="mt-2 text-xs font-semibold text-[var(--muted)]">Batas awal per server</p></div>
      ...
      {:else}
        <div class="divide-y divide-[var(--border)]">
          {#each streams as stream}
            <div class="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto_auto] sm:items-center sm:px-6">
