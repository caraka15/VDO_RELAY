...
<div class="mx-auto w-full max-w-5xl px-4 py-5 pb-8 sm:px-6 sm:py-7 lg:px-8">
  ...
  <div class="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(290px,0.65fr)]">
    <section class="panel p-5 sm:p-6" aria-labelledby="output-heading">
      ...
      <p class="mt-2 max-w-xl text-sm leading-6 text-[var(--muted)]">Resolusi dan FPS di sini adalah target track final. Jika sensor kamera 4:3, browser diminta crop-and-scale ke rasio 16:9/9:16 tanpa canvas; ukuran aktual dapat mengikuti kemampuan perangkat.</p>
      ...
      <p class="mt-3 text-xs font-semibold leading-5 text-[var(--faint)]">Codec WebRTC lain: {codecs.filter((codec) => !codec.srtCompatible).map((codec) => `${codec.label} tidak dipakai`).join(" Â· ") || "belum dibaca"}. V1 hanya mengunci H.264 atau H.265 yang terdeteksi efisien daya.</p>
