<script lang="ts">
  ...
  async function checkCameraProfiles(key: string, deviceId: string | undefined, device: CameraDevice | undefined) {
    ...
    cameraProfiles = [];
    try {
      const profiles = await probeCameraProfiles(deviceId, device);
      if (sequence !== profileProbeSequence || key !== profileProbeKey) return;
      cameraProfiles = profiles;
      if (profiles.length === 0) profileError = "Kamera belum melaporkan profil resolusi/FPS yang bisa dipakai. Pilih sumber kamera lain atau tekan Deteksi lagi.";
    } catch (probeError) {
...
<div class="mx-auto w-full max-w-5xl px-4 py-5 pb-8 sm:px-6 sm:py-7 lg:px-8">
  <header class="mb-6 flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      ...
      <p class="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">Pilih preferensi kamera. Chrome akan memilih track native terdekat sesuai perangkat dan orientasi saat Start; video dikirim langsung ke WHIP tanpa canvas.</p>
    </div>
  ...
  <div class="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(290px,0.65fr)]">
    <section class="panel p-5 sm:p-6" aria-labelledby="output-heading">
      <div class="mb-5 flex items-center gap-2 text-[var(--accent)]"><ShieldCheck size={18} /><span class="mono text-xs font-bold uppercase tracking-[0.14em]">STEP 01 / CAMERA PROFILE</span></div>
      <h2 id="output-heading" class="text-xl font-extrabold">Codec dan preferensi kamera</h2>
      <p class="mt-2 max-w-xl text-sm leading-6 text-[var(--muted)]">Ukuran di bawah adalah target preferensi, bukan ukuran yang dipaksa. Jika HP sedang portrait, Chrome dapat mengembalikan track portrait; jika sedang landscape, track tetap landscape.</p>
      ...
      <div class="mt-5 grid gap-4 sm:grid-cols-2">
        <div><label for="resolution" class="mb-2 block text-sm font-bold">Preferensi resolusi</label><select id="resolution" class="field w-full px-3" bind:value={resolutionKey} disabled={profileChecking || availableResolutions.length === 0}>{#if availableResolutions.length === 0}<option value="">{profileChecking ? "Memeriksa kamera..." : "Profil kamera belum terbaca"}</option>{/if}{#each availableResolutions as option}<option value={resolutionValue(option)}>{resolutionValue(option)} · {option.label}</option>{/each}</select></div>
        <div><label for="fps" class="mb-2 block text-sm font-bold">Preferensi FPS</label><select id="fps" class="field w-full px-3" bind:value={fps} disabled={profileChecking || availableFps.length === 0}>{#if availableFps.length === 0}<option value={fps}>{profileChecking ? "Memeriksa kamera..." : "FPS kamera belum terbaca"}</option>{/if}{#each availableFps as option}<option value={option}>{option} FPS</option>{/each}</select></div>
      </div>

      <div class="mt-4 border border-[var(--border)] bg-[var(--surface-raised)] p-4">
        <div class="flex items-start gap-3 text-sm font-extrabold"><Camera size={17} class="mt-0.5 shrink-0 text-[var(--accent)]" /><div><p>Preferensi kamera: {outputWidth ? `${outputWidth} × ${outputHeight} · ${fps} FPS` : "belum ada"}</p><p class="mt-1 text-xs font-semibold text-[var(--muted)]">Track kamera dikirim langsung ke WHIP. Rasio dan orientasi aktual dibaca setelah kamera dimulai; server tidak melakukan transcoding.</p></div></div>
        <div class="mt-3 flex items-start gap-2 border-t border-[var(--border)] pt-3 text-xs font-bold" aria-live="polite">
          {#if profileChecking}<Activity size={15} class="mt-0.5 shrink-0 animate-pulse text-[var(--warning)]" /><span class="text-[var(--warning)]">Membaca profil kamera...</span>
          {:else if profileSupported}<Check size={15} class="mt-0.5 shrink-0 text-[var(--success)]" /><span class="text-[var(--success)]">Preferensi ini tersedia; Chrome akan memilih track native terdekat saat Start.</span>
          {:else}<X size={15} class="mt-0.5 shrink-0 text-[var(--danger)]" /><span class="text-[var(--danger)]">Preferensi ini belum tersedia. Pilih opsi lain.</span>{/if}
        </div>
    ...
    <aside class="space-y-5">
      <section class="panel p-5" aria-labelledby="camera-heading">
        ...
        <p class="mt-4 border-t border-[var(--border)] pt-4 text-xs font-semibold leading-5 text-[var(--muted)]">Resolusi/FPS di sebelah kiri adalah preferensi getUserMedia. Angka max di atas adalah kemampuan sumber kamera yang dilaporkan browser; ukuran aktual tampil di halaman live.</p>
      </section>
